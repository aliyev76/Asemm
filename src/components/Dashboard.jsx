...
const Dashboard = ({ activeTab }) => {
  ...
  const t = translations.tr.dashboard; 
  ...
  React.useEffect(() => {
    ...
    // Ürün Listesi Zorunlu Güncelleme (Versiyon 2.1)
  ...
  const handleUpdateStartTime = (id, isVip, timeStr) => {
    ...
    setter(prev => prev.map(t => {
      ...
      const now = new Date();
      if (newStart > now) newStart.setDate(newStart.getDate() - 1);
  ...
  const handleEndDay = () => {
    ...
    const discountCount = logs.reduce((sum, l) => sum + (parseFloat(l.discount) || 0), 0);
    ...
    const debtorLogs = logs.filter(l => l.debt > 0).map(l => ({
      tableName: l.tableName, debt: l.debt, note: l.note || 'İsimsiz', timestamp: l.timestamp
    }));
        ...
        {['dashboard', 'masalar', 'vips'].includes(activeTab) && (
          <div className="filters-container">
            ...
            <div className="filter-group">
              ...
              <div className="chips-container">
                <button className={`game-chip ${selectedGame === null ? 'active' : ''}`} onClick={() => setSelectedGame(null)}>{t.all}</button>
                {allGames.map(game => (
                  <button key={game} className={`game-chip ${selectedGame === game ? 'active' : ''}`} onClick={() => setSelectedGame(game)}>{game}</button>
                ))}
              </div>
            </div>
          </div>
  ...
  const selectedTable = useMemo(() => {
    ...
    if (!table) table = [...tables, ...vips].find(t => String(t.id) === String(selectedTableInfo.id));
    return table;
